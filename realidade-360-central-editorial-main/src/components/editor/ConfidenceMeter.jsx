...
export default function ConfidenceMeter({ score, band }) {
  ...
  return <div className="rounded-2xl border border-white/7 bg-[#0b1424] p-4">
    <div className="flex items-center justify-between"><span className="text-xs font-semibold text-slate-400">Confiança documental</span><span className="text-2xl font-bold text-white">{s}</span></div>
    ...
    <p className="mt-2 text-[11px] leading-relaxed text-slate-600">Mede o suporte documental das fontes vinculadas — não a probabilidade de verdade. Revisão humana necessária.</p>
  </div>;
}